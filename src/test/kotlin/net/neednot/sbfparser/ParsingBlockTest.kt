...
class ParsingBlockTest {
    ...
    fun parseTimestampTest() {
        val data = "ff ff ff ff ff ff".replace(" ", "").hexToByteArray()
        val buffer = ByteBuffer.wrap(data).order(ByteOrder.LITTLE_ENDIAN)
    ...
    @Test
    fun parseBodyTest() {
        val data = "04 00 0b 00 01 00 15 0a 00 00 24 40 34 b1 f2 0f 18 00 ff ff ff ff ff ff 04 00 0b 00 01 00 15 0a 00 00".replace(" ", "").hexToByteArray()
        val buffer = ByteBuffer.wrap(data).order(ByteOrder.LITTLE_ENDIAN)
